...
ggplot(subset(data_weekly, !is.na(continent)), aes(x = week, y = depressed_weekly_flip, colour = continent)) +
  geom_line(size = 1.5) +
...
ggplot(subset(data_weekly, !is.na(continent)), aes(x = week, y = anxious_weekly_flip, colour = continent)) +
  geom_line(size = 1.5) +
  theme(legend.position = "top") +
...
nrow(data %>% drop_na(gender_grouped, continent, anxious) %>% filter(gender_grouped == 1)) # -> 22063709
nrow(data %>% drop_na(gender_grouped, continent, anxious) %>% filter(gender_grouped == 2)) # -> 21130993
nrow(data %>% drop_na(gender_grouped, continent, anxious)) # -> 43194702
...
dt_decrease_5000 <- dt_decrease %>%
  filter(n >= 5000)

nrow(dt_decrease_5000)


###### Depression

## Find countries that have highest and lowest increase of variance
dt_increase_dep_var <- dt_decrease_5000 %>%
